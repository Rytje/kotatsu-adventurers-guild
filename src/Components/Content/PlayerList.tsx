  ...
  return (
    <>
      ...
      {profileState.map((element, index) => {
        return <PlayerCard key={index} avatar={element.avatar} name={element.name} ign={element.ign} lvl={element.lvl} uid={element.uid} server={element.server} msg={element.msg} favChar={element.favChar} wantedChar={element.wantedChar} pinCode={element.pinCode} updOn={element.updatedOn} docId={docIdState[index]} husbando={element.husbando} waifu={element.waifu} kiddo={element.kiddo} artifact={element.artifact} />
      })}
    </>
  )
}